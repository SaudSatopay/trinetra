...
- **Benchmark.** 29 labelled scenarios (`backend/benchmark.py`): **14 genuine compound hazards** (varying zone, lead gas, ramp speed, in-zone vs adjacent ignition) and **15 hard negatives** — gas releases with no ignition/personnel, permitted hot-work in clean air, transient gas spikes, a single-sample O2-sensor dropout in an occupied zone, and **inerted zones where all three compound factors are present but combustion is impossible**. "Detected" = Trinetra raises a compound alert at ELEVATED+; the baseline "detects" when any gas crosses its setpoint.
...
Recall holds at 100% and false positives stay at **3.3%** on scenarios the engine never saw — the thresholds generalize, they are not overfit to the curated 29. (This is still the digital twin, not live plant telemetry; real-plant data enters the *same* engine unchanged through the `/api/ingest` connector.)

> Reproduce: `cd backend && python test_generalization.py`

## Real-incident replay — reconstructed from two real inquiries (×2)
...
- The 29 scenarios were authored by us, but deliberately include **hard negatives** the engine must reject — including three *inerted* cases that carry all three compound factors yet are physically safe, and a single-sample O2-sensor dropout that must not fabricate an asphyxiation alert. The discrimination (0% false-positive across them) is the substantive result, not the recall.
...
A tenth case — a **missing CCTV feed** — is handled at the API layer, not the engine: personnel come from the permit-to-work system, so `/api/vision` degrades to an error object and the compound decision is unaffected.
...
| False-positive discipline | 0% on 15 hard negatives (incl. inerted + an O2-sensor dropout) |
