...
// ---- palette ----
const BG = "0b0f14", PANEL = "10161e", PANEL2 = "0c1218", LINE = "26313e",
  TEAL = "34d3bd", RED = "fb3b4e", AMBER = "e8862f", GREEN = "3a9d8c",
  TEXT = "c8d3de", DIM = "6b7888", BRIGHT = "eaf2fa", LEGACY = "5a6775";
...
function kicker(s, t, x = 0.7, y = 0.55) {
  s.addText(t, { x, y, w: 8, h: 0.3, fontFace: MONO, fontSize: 12, color: TEAL, charSpacing: 3, margin: 0 });
}
...
function mark(s, cx, cy, sc) { // triangle + eye
  const t = (a, b, c, d) => s.addShape(p.shapes.LINE, { x: a, y: b, w: c - a, h: d - b, line: { color: TEAL, width: 2 } });
  t(cx, cy - sc, cx + sc, cy + sc); t(cx + sc, cy + sc, cx - sc, cy + sc); t(cx - sc, cy + sc, cx, cy - sc);
  s.addShape(p.shapes.OVAL, { x: cx - sc * 0.42, y: cy + sc * 0.05, w: sc * 0.84, h: sc * 0.55, fill: { type: "none" }, line: { color: TEAL, width: 1.5 } });
  dot(s, cx - sc * 0.07, cy + sc * 0.22, TEAL, sc * 0.09);
}
...
// ============ S1 — TITLE ============
{
  ...
  s.addText("THE THIRD EYE THAT SEES THE DANGER NO SINGLE SENSOR CAN", { x: 0, y: 3.7, w: W, h: 0.4, fontFace: MONO, fontSize: 13, color: TEAL, align: "center", charSpacing: 2, margin: 0 });
  ...
  s.addText([
    { text: "100% compound recall", options: { color: TEAL } }, { text: "    ·    ", options: { color: DIM } },
    { text: "0% false-positive", options: { color: TEAL } }, { text: "    ·    ", options: { color: DIM } },
    { text: "7.4 min mean early-warning", options: { color: TEAL } },
...
// ============ S3 — INSIGHT ============
{
  ...
  const items = [["Rising flammable gas", "still below its alarm", GREEN], ["Active hot-work permit", "an ignition source", GREEN], ["Personnel present", "inside a confined space", GREEN]];
  let x = 0.7;
  items.forEach(([t, sub, c]) => {
    ...
    s.addText("normal", { x: x + 0.25, y: 3.75, w: 2.85, h: 0.3, fontFace: MONO, fontSize: 10, color: GREEN, margin: 0 });
    x += 3.55;
  });
...
// ============ S4 — DEMO MOMENT ============
{
  ...
  dot(s, 1.0, 3.45, GREEN, 0.16);
  s.addText("ALL CLEAR", { x: 1.5, y: 3.25, w: 4, h: 0.6, fontFace: HEAD, fontSize: 30, bold: true, color: GREEN, margin: 0 });
  ...
  // center
  panel(s, 5.95, 3.25, 1.45, 1.8, PANEL2, TEAL);
  s.addText("+6", { x: 5.95, y: 3.45, w: 1.45, h: 0.8, fontFace: HEAD, fontSize: 38, bold: true, color: TEAL, align: "center", margin: 0 });
  s.addText("MIN", { x: 5.95, y: 4.25, w: 1.45, h: 0.3, fontFace: MONO, fontSize: 12, color: TEAL, align: "center", margin: 0 });
  s.addText("early", { x: 5.95, y: 4.6, w: 1.45, h: 0.3, fontFace: MONO, fontSize: 10, color: DIM, align: "center", margin: 0 });
  // trinetra
  panel(s, 7.63, 2.2, 5.0, 3.9, "190f12", RED);
  s.addText("TRINETRA COMPOUND AI", { x: 7.93, y: 2.5, w: 4.4, h: 0.3, fontFace: MONO, fontSize: 12, color: TEAL, charSpacing: 1, margin: 0 });
...
// ============ S5 — ARCHITECTURE ============
{
  ...
  panel(s, 0.7, 5.5, 11.93, 1.0, PANEL2, TEAL);
  s.addText([
    { text: "Hybrid by design:  ", options: { color: TEAL, bold: true } },
...
// ============ S6 — DISASTER MEMORY ============
{
  ...
  others.forEach(([n, l]) => {
    ...
    s.addText(n, { x: x + 0.2, y: 5.2, w: 0.9, h: 0.65, fontFace: HEAD, fontSize: 22, bold: true, color: TEAL, margin: 0, valign: "middle" });
...
// ============ S7 — RESPONSE ============
{
  ...
  s.addText("AUTO-DRAFTED INCIDENT REPORT", { x: 1.0, y: 2.55, w: 5.5, h: 0.3, fontFace: MONO, fontSize: 11, color: TEAL, charSpacing: 1, margin: 0 });
  s.addText([
    ...
    { text: "    Factory Act 1948 §36 / §37 / §38", options: { color: TEAL, breakLine: true } },
    { text: "    OISD-STD-105 (Work Permit System)", options: { color: TEAL, breakLine: true } },
  ...
  // alert + actions
  panel(s, 7.0, 2.3, 5.63, 2.05, "1a1407", AMBER);
...
// ============ S8 — PROOF ============
{
  ...
  const stats = [["100%", "compound recall", "14 / 14 hazards caught", TEAL], ["0%", "false-positive", "0 / 11 hard negatives", TEAL], ["7.4 min", "mean early-warning", "vs single-sensor baseline", AMBER]];
  let x = 0.7;
  ...
  const tl = [[1.2, "t0", "normal", DIM], [4.4, "t8", "Trinetra: COMPOUND", TEAL], [9.6, "t14", "Legacy: gas alarm", RED]];
  tl.forEach(([px, t, l, c]) => {
...
// ============ S9 — SCALE ============
{
  ...
  panel(s, 7.9, 2.35, 4.73, 4.04, PANEL2, TEAL);
  s.addText("WHERE IT DEPLOYS", { x: 8.2, y: 2.6, w: 4, h: 0.3, fontFace: MONO, fontSize: 11, color: TEAL, charSpacing: 2, margin: 0 });
...
// ============ S10 — CLOSE ============
{
  ...
  s.addText([
    ...
    { text: "0% false-positive", options: { color: TEAL } }, { text: "   ·   ", options: { color: DIM } },
    { text: "7.4 min early-warning", options: { color: TEAL } },
