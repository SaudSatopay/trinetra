...
- **"Why won't it cry wolf?"** → 15 hard-negative scenarios — including the toughest, where rising gas, a hot-work permit and crew are ALL present but the zone is inerted (no oxidizer to burn), which a "three boxes ticked" rule gets wrong — at 0% false compound alerts. Plus nine fault-mode checks (stuck sensor, noise spike, delayed permit, faulty-low and transient O2, missing CCTV) in `test_robustness.py`.
